...
import { ArrowLeft, ArrowRight, Check, CheckCircle2, ChevronRight, ClipboardList, HeartPulse, LoaderCircle, Mic, MicOff, Pencil, RotateCcw, ShieldAlert, UserRound, Volume2 } from 'lucide-react'
...
import type { CaseDraft, ConsentRecord, IntakeAnalysis, IntakeQuestion, Patient, PatientExperienceConfig, PatientLookupMethod } from '../../types/clinical'
...
export function PatientExperience({ config, language, voice, ayush, active, onNewSession }: Props) {
  ...
  const finish = async () => {
    ...
    try {
      ...
      const result = await submitCase({ id: caseId.current, patientId: patient.id, status: 'draft', chiefComplaint: complaint, hpi: summary, pastHistory: allAnswers.history ?? '', drugAndAllergy: allAnswers.medicines ?? '', familyHistory: '', ros: analysis.summary, answers: allAnswers, language, pathway: ayush ? 'ayush' : 'general', urgent: analysis.urgent })
      setSubmitted(result)
  ...
  const steps = [t('आपकी परेशानी', 'Your concern'), t('आपकी जानकारी', 'Your details'), t('आपकी अनुमति', 'Permissions'), t('समीक्षा', 'Review')]
